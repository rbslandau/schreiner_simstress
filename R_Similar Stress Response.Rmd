...
# Similar dynamics of microbial functional stress response across biogeographical regions
...
data_all_OMD_control <-  melt(data_all_OMD_control_u, id = c("fungicide", "cycle", "region",
                                                             "Number.replicate"))

data_all_OMD_control <- dcast(data_all_OMD_control, fungicide + cycle + region ~ variable, mean)
...
data_all_OMD_perc <- merge(data_all_OMD_control, data_all_OMD_trt, by=c("cycle", "region",
                                                                        "Number.replicate"))
...
```{r}
mod1_all_OMD_perc <- lmer(diff ~ region + cycle + (region | Number.replicate),   
...
Factor region is not significant, while the cycle displays a significant relationship.
